...
# xAIかOpenAIかを自動判別してモデルも自動決定
if api_key.startswith("xai-"):
    client = OpenAI(
        api_key=api_key,
        base_url="https://api.x.ai/v1"
    )
    MODEL = "grok-2-1212"   # xAIで現在最も安定しているモデル
    st.sidebar.info("xAI Grok APIを使用中（grok-2-1212）")
else:
    client = OpenAI(api_key=api_key)
    MODEL = "gpt-4o-mini"   # OpenAIで安くて高性能
...
st.divider()
st.caption("xAI Grok API / OpenAI API 両対応済み（モデル自動選択・選択不要）")
