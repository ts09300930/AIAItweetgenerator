...
# =====================
# APIキー取得（Streamlit Cloud + ローカル両対応）
# =====================
with st.sidebar:
    st.header("⚙️ 設定")
    ...
    if "OPENAI_API_KEY" in st.secrets:
        ...
        api_key = st.text_input("OpenAI APIキー", type="password", 
                               help="Cloudにデプロイする場合はSecretsに設定してください")
        if not api_key:
            st.warning("APIキーを入力するか、Secretsを設定してください")
            st.stop()
...
st.divider()
st.caption("このツールは「あなたがAIに指示を出す」ことを重視した設計です。Secrets対応済み（Streamlit Cloud推奨）")
