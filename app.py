...
with st.sidebar:
    ...
    model = st.selectbox("モデル", ["grok-2", "grok-beta", "gpt-4o-mini", "gpt-4o"], index=0)
...
if api_key.startswith("xai-"):
    client = OpenAI(
        api_key=api_key,
        base_url="https://api.x.ai/v1"   # ← これが大事！
    )
    st.sidebar.info("xAI Grok APIを使用中")
else:
    client = OpenAI(api_key=api_key)
    st.sidebar.info("OpenAI APIを使用中")
...
st.divider()
st.caption("xAI Grok API / OpenAI API 両対応済み")
